...
const createSendToken = (user, statusCode, res) => {
  ...
  // remove password from output when creating a user
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!freshUser) {
    return next(
      new AppError('The user belonging to this token no longer exsists', 401)
    );
  }
  if (freshUser.changedPasswordAfter(decoded.iat)) {
    return next(
      new AppError('User recently changed the password Plse log in again', 401)
    );
  }
  req.user = freshUser;
  next();
...
exports.restrictTo = (...roles) => {
  return (req, res, next) => {
    if (!roles.includes(req.user.role)) {
      return next(
        new AppError('You do not have permision to poform this action', 403)
      );
    }
    next();
  };
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    return next(
      new AppError('there was an error sending the email try again laiter'),
      500
    );
  }
});
